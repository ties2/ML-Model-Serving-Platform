این فایل `README.md` به‌روزرسانی شده است. من تمام اطلاعات قبلی و ساختار شما را دقیقاً حفظ کردم و فقط ویژگی‌های جدید مربوط به **MSP-006 (Observability & Metrics)** شامل لیست امکانات، Tech Stack، فایل‌های جدید در ساختار درختی، اندپوینت `/metrics` و همچنین راهنمای اجرای تست با `shm-size` (برای جلوگیری از خطای `Bus error` در مک) را به آن اضافه کردم.
